...
const panelStyles = (theme: Theme) =>
  createStyles({
    ...
    header: {
      ...
      padding: `${theme.spacing(1) / 2}px ${theme.spacing(2)}px ${theme.spacing(
        1,
      ) / 2}px ${theme.spacing(1) * 1.5}px`,
    },
    ...
    body: {
      padding: `${theme.spacing(1)}px ${theme.spacing(1) * 1.5}px`,
    },
  });
      ...
      actions.forEach((action: any, index) => {
        actionEls.push(
          React.cloneElement(action, {
            className:
              action.type === IconButton ? classes.headerToolboxButton : "",
            key: `ACTION-${index}`,
          }),
        );
      });
          ...
          {title && (
            <div
              className={classNames(
                ...
                this.props.variant || classes.headerForNonBordered,
              )}
            >
              <Typography
